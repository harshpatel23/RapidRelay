#!/home/tanay/Projects/BugBox/venv1/bin/python
...
def compress_data_to_file_from(table):
    ...
    try:
        ...
        with open(f'{base_dir}/{raw_files_dir}/{table}_{current_time}', 'wb') as bin_file:
        ...
        with open(f'{base_dir}/{compressed_files_dir}/{table}_{current_time}', 'wb') as bin_file:
...
if is_connected(host) and not is_already_running():
    ...
    print('connected')
    try:
        for filename in os.listdir(f'{base_dir}/{compressed_files_dir}'):
            print(filename)
            # send file to cloud
